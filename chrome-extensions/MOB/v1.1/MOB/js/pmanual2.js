...
function fufu(e) {
	...
	if ( (e.keyCode == 65) || 
	    ...
	    { //65 a 78 n 83 s 72 h 70 f 68 d
			console.log(String.fromCharCode(e.which));
			if (lastLog) {
				if (e.keyCode==65) lastLog.setLabelByUser("ARTICLE");
				if (e.keyCode==78) lastLog.setLabelByUser("NAV");
				if (e.keyCode==83) lastLog.setLabelByUser("SECTION");
				if (e.keyCode==72) lastLog.setLabelByUser("HEADER");
				if (e.keyCode==70) lastLog.setLabelByUser("FOOTER");
				if (e.keyCode==68) lastLog.setLabelByUser("ASIDE");
				lastLog.updateBlock();
			}
	}
...
function carga(e) {
		...
		if (!cargado) {
			...
			if (is_html5()){
				document.write('Sorry this page is already in HTML5 format');
				return false;
			}
			
			//~ originalSource = $('html').html();
			
			originalSource = document.documentElement.outerHTML;
			//~ originalSource = document.body.innerHTML;
			pac = 7;
			pac = parseFloat(prompt("Segment with which granularity: 0 - 10 \nGive a number\n small blocks 0 <--> 10 bigger blocks",pac));
			if (!pac) return;
			sourceurl = location.href;
			startSegmentation(window,pac,50,"wprima",sourceurl);
...
function update_marco() {
    ...
    s+="<td class='bommarco'><img class='bommarco' src='"+ mylogo +"' width='64'></td>";
    s+="<td class='bommarco'>BOM Version:"+bomversion+"<br class='bommarco'>";
    s+="pAC:"+pac+"<br class='bommarco'>";
	...
	s+="<table  class='bommarco' cellspacing=0 cellpadding=0 border=0 align='center'>";
	blocks = [];
	updateBlockList(page);
	s+=blocks.length+" blocks";	
	s+="</table>";
	s+="<select name='bomtable' id='bomtable' class='bommarco'><option name='mob' value='mob' class='bommarco'>MOB</option><option name='bom' value='bom' class='bommarco'>BOM</option></select>";
	...
	s+="</tr></table>";
	s+="<form class='bommarco' id='outlinedata' name='outlinedata' method='post' action='http://localhost/BOM/html5res.php' target='_blank'><input type='hidden' class='bommarco' name='original' id='outoriginal'><input type='hidden' class='bommarco' name='descriptor' id='outprima'><input type='hidden' class='bommarco' name='urlsrc' id='urlsrc'><input type='hidden' class='bommarco' name='target' id='target'><input type='hidden' class='bommarco' name='pac' id='pac'></form>'";
	marco.setContent(s);
...
function update_marco_submit() {
	...
	document.getElementById('outoriginal').value = originalSource;
	document.getElementById('outprima').value = getDescriptorSorted(page);
	document.getElementById('urlsrc').value = document.URL;
	document.getElementById('pac').value = pac;
	e=document.getElementById('bomtable');
	document.getElementById('target').value = e.options[e.selectedIndex].value;
	document.getElementById('outlinedata').submit();
}
...
function insertNew(e) {
	...
	var go = elist[elist.length-2];
	var geo;
	trail =[];
	dale=true;
	k=3;
	while (dale) {
		geo = processGeometricObject(go);
		if (!geo) {
			if (elist[elist.length-k]) {
				go = elist[elist.length-k];
				k++;
			} else {
				dale=false;
			}
	...
	if (geo) {
		...
		console.log("the geometric object is invalid");
	}
}
...
function dale(e) {
	debug("CLICK1 ", "merge:",merging,"edit:",editing,"insert",insert,"lb:",lastBlock,"ll:",lastLog,"e:",e);
	if (insertcustom) {
	...
	if ( !editing && 
		...
		if (!lastLog) return;
		document.getElementById("mobstatus").innerHTML = "<b>"+lastLog.getId()+"("+lastLog.id+")"+"</b><br><b>Granularity:</b>: "+lastLog.gran.toFixed(4) + "<br><b>HTMLCover:</b> "+lastLog.htmlcover+"<br><b>WordCover:</b> "+lastLog.wordcover+" ("+(lastLog.wordcover/page.wordcover).toFixed(2)+"%)<br><b>Importance:</b>"+lastLog.importance;
...
function removeBlock() {
	if (lastLog.type == "PAGE" || lastLog.label=="PAGE") return;
