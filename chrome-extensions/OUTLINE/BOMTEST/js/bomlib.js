...
var contentList = ["SPAN", "A", "LI", "DT", "DD", "H1", "H2", "H3", "H4", "H5", "H6","IMG", "INS", "INPUT", "P"];
...
function logicalObject(obj) {
    ...
    this.updateBlock = function() {
        ...
        this.block.innerHTML = "<span visited='true' class='bomauxtext' style='opacity:1;color:black;font-size:12pt;background-color:"+bg+";color:white'>B" + this.getId() + "</span>";
...
function getRecord(obj,srcurl) {
	...
	if (obj.terminal()) {
		ret = [];
		for (var i=0;i<obj.geometricObjects.length;i++) {
			if (obj.geometricObjects[i]) {
				rec = []
				g = obj.geometricObjects[i];
				rec.push("BOM1.1");
				rec.push(sniff())
				rec.push(window.location.href)
				rec.push(documentDim().w)
				rec.push(documentDim().h)
				rec.push(georoot.countCover() )
				rec.push(ac)
				rec.push("L"+obj.getId())
				rec.push(obj.dim.x)
				rec.push(obj.dim.y)
				rec.push(obj.dim.w)
				rec.push(obj.dim.h)
				rec.push(obj.countCover())
				rec.push(g.element.tagName)
				sid = "";
				if (g.element.getAttribute('id')) sid=g.element.getAttribute('id');
				if (g.element.getAttribute('name')) sid+=" "+g.element.getAttribute('name');
				if (g.element.className) sid+=" "+g.element.className;
				rec.push("'"+sid+"'")
				rec.push("images")
				rec.push("text")
				rect = getRect(g.element);
				rec.push(rect.x)
				rec.push(rect.y)
				rec.push(rect.w)
				rec.push(rect.h)
				rec.push("FLOW") //category
				rec.push("FLOW") //content_below
				var base = window.location.href
				var aurl=[];
				$(g.element).find("img").each(function(img){
					aurl.push(new URL($(this).attr("src"), base));
				})
				rec.push(aurl.join("','")); //imagenes
				txt = originalText;
				//~ txt = $(g.element).parent().text();
				txt = txt.toString().replace(/\t/g, '').replace(/\\t/g, '').replace(/\\n\\n/g, '\\n').replace(/  /g, ' ').replace(/\s/g, ' ');
				rec.push(txt); //texto
				rec.push(obj.label); //texto
				rec.push(obj.gran); //texto
